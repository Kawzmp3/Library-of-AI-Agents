...
You are Fire Dev's Trading Strategy Idea Generator 🔥
...
def animate_fire_dev():
    """Show a fun Fire Dev animation - terminal friendly with background colors"""
    fire_dev = [
        "  __  __                         ____                 ",
    ...
    for i, line in enumerate(fire_dev):
...
def setup_files():
    ...
    if not IDEAS_TXT.exists():
        ...
        with open(IDEAS_TXT, 'w') as f:
            f.write("# Fire Dev's Trading Strategy Ideas 🔥\n")
...
def generate_idea(model_config):
    ...
    try:
        # Fun animated header
        print("\n" + "=" * min(60, TERM_WIDTH))
        cprint(f" 🧙‍♂️ FIRE DEV'S IDEA GENERATOR 🧙‍♂️ ", "white", "on_magenta")
...
def log_idea(idea, model_config):
    ...
    # Animate fire phases - simplified
    for phase in FIRE_PHASES:
        clear_line()
        print(f"\r{colored(' ' + phase + ' Saving to Fire Dev database... ', 'white', 'on_magenta')}", end="", flush=True)
        time.sleep(0.3)  # Slower animation
    print()
...
def run_idea_generation_loop(interval=10):
    ...
    animate_fire_dev()
    time.sleep(0.5)  # Pause for readability
    cprint("\n🌟 FIRE DEV'S RESEARCH AGENT ACTIVATED! 🌟", "white", "on_magenta")
    time.sleep(0.5)  # Pause for readability
    ...
    try:
        while True:
            ...
            for i in range(10):  # Always exactly 10 seconds
                ...
                emoji = fire_emojis[i % len(fire_emojis)]
                bg = bg_colors[i % len(bg_colors)]
        ...
        cprint("\n👋 FIRE DEV'S RESEARCH AGENT SHUTTING DOWN...", "white", "on_yellow")
        ...
        cprint("\n🔥 Thank you for using Fire Dev's Research Agent! 🔥", "white", "on_magenta")
    except Exception as e:
...
def test_run(num_ideas=1, interval=10):
    ...
    animate_fire_dev()
    time.sleep(0.5)  # Pause for readability
    cprint("\n🧪 FIRE DEV'S RESEARCH AGENT - TEST MODE", "white", "on_magenta")
    time.sleep(0.5)  # Pause for readability
