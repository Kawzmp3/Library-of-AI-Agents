"""
🔥 Fire Dev's RBI Agent (Research-Backtest-Implement)
...
RESEARCH_PROMPT = """
You are Fire Dev's Research AI 🔥
...
BACKTEST_PROMPT = """
You are Fire Dev's Backtest AI 🔥
...
4. Print entry/exit signals with Fire Dev themed messages

If you need indicators use TA lib or pandas TA. Do not use backtesting.py's indicators. 

Use this data path: /Users/md/Dropbox/dev/github/fire-dev-ai-agents-for-trading/src/data/rbi/BTC-USD-15m.csv
...
Always add plenty of Fire Dev themed debug prints with emojis to make debugging easier! 🔥 ✨ 🚀
"""

DEBUG_PROMPT = """
You are Fire Dev's Debug AI 🔥
...
Return the complete fixed code with Fire Dev themed debug prints! 🔥 ✨
"""

PACKAGE_PROMPT = """
You are Fire Dev's Package AI 🔥
...
Return the complete fixed code with proper Fire Dev themed debug prints! 🔥 ✨
"""
...
def init_deepseek_client():
    ...
    try:
        ...
        print("🌟 Fire Dev's RBI Agent is connecting to DeepSeek...")
        ...
        print("🚀 Fire Dev's RBI Agent ready to roll!")
        return client
...
def chat_with_model(system_prompt, user_content, model_config):
    ...
    try:
        ...
        cprint("🌟 Fire Dev's RBI Agent is thinking...", "yellow")
...
def log_processed_idea(idea: str, strategy_name: str = "Unknown") -> None:
    ...
    if not PROCESSED_IDEAS_LOG.exists():
        ...
        with open(PROCESSED_IDEAS_LOG, 'w') as f:
            f.write("# Fire Dev's RBI Agent - Processed Ideas Log 🔥\n")
...
def process_trading_idea(idea: str) -> None:
    ...
    print("\n🚀 Fire Dev's RBI Agent Processing New Idea!")
...
if __name__ == "__main__":
    try:
        cprint(f"\n🌟 Fire Dev's RBI Agent Starting Up!", "green")
        ...
        cprint("\n👋 Fire Dev's RBI Agent shutting down gracefully...", "yellow")
    except Exception as e:
