'''
🔥 Fire Dev's Solana Analysis Agent - Built with love by Fire Dev 🚀
...
class SolanaAnalyzer:
    def __init__(self):
        """🔥 Fire Dev's Solana Analyzer - Built with love by Fire Dev 🚀"""
        self.api_key = os.getenv('FIREDEV_API_KEY')
    ...
    def analyze_token(self, token_address):
        """Analyze a single token using Fire Dev's criteria"""
        try:
    ...
    def display_top_pick(self, token_data):
        """Display a top pick with Fire Dev style 🔥"""
        random_emoji = random.choice(ANALYSIS_EMOJIS)
        random_bg = random.choice(BACKGROUND_COLORS)
        
        print(f"\n{colored(f'{random_emoji} FIRE DEV AI AGENT TOP PICK', 'white', random_bg)}")
    ...
    def run_analysis(self):
        """Main analysis loop"""
        print("\n🔍 Starting Fire Dev's AI Analysis Agent...")
        ...
        while True:
            ...
            print(f"\n😴 Fire Dev's AI Agent sleeping for {CHECK_INTERVAL/60:.1f} minutes...")
